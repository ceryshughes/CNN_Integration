...
class WaveCNN():
    ...
    def __init__(self, kernel_len=25, dim=64, use_batchnorm=True, name="cnn", num_classes=2):
        ...
        # Layer 0
        # [16384, 1] -> [4096, 64]
        self.model.add(layers.Conv1D(dim, kernel_len,4,padding="same"))
        if use_batchnorm:
        ...
        # Layer 1
        # [4096, 64] -> [1024, 128]
        self.model.add(layers.Conv1D(dim * 2, kernel_len, 4, padding="same"))
        if use_batchnorm:
            self.model.add(layers.BatchNormalization())
        self.model.add(layers.LeakyReLU(alpha=0.2))
        ...
        # Layer 2
        # [1024, 128] -> [256, 256]
        self.model.add(layers.Conv1D(dim*4,kernel_len,4, padding="same"))
        if use_batchnorm:
             self.model.add(layers.BatchNormalization())
        self.model.add(layers.LeakyReLU(alpha=0.2))
        ...
        # Layer 3
        # [256, 256] -> [64, 512]
        self.model.add(layers.Conv1D(dim * 8, kernel_len, 4, padding="same"))
        if use_batchnorm:
            self.model.add(layers.BatchNormalization())
        self.model.add(layers.LeakyReLU(alpha=0.2))
        ...
        # Layer 4
        # [64, 512] -> [16, 1024]
        self.model.add(layers.Conv1D(dim * 16, kernel_len, 4, padding="same"))
        if use_batchnorm:
            self.model.add(layers.BatchNormalization())
        self.model.add(layers.LeakyReLU(alpha=0.2))
        ...
        # just 1D, so shape -1?
        self.model.add(layers.Reshape((1, -1),name="hidden_rep"))
